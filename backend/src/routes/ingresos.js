...
router.post('/', upload.single('imagen'), (req, res) => {
  ...
  db.run(
    `INSERT INTO ingresos (
      fecha, proveedor, descripcion, cantidad, unidad, ubicacion,
      observaciones, imagen, largo, ancho, alto, cbm
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    ...
    err => {
      ...
      return res.send('Ingreso registrado con CBM');
    }
  );
});
...
router.post('/multiple', upload.array('imagenes', 5), (req, res) => {
  ...
  const cbm = (l_m * a_m * h_m).toFixed(3);
  const pesoKg = peso === undefined || peso === '' ? null : parseFloat(peso);

  db.run(
    `INSERT INTO ingresos (
      fecha, proveedor, descripcion, cantidad, unidad, ubicacion,
      observaciones, imagen, largo, ancho, alto, cbm, peso
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      fecha,
      proveedor,
